    ...
    const fetchLinkHistory = async () => {
      try {
        const response = await axios.get(
          `https://scissor-mbob.onrender.com/link-history/${user.id}`
        );
          ...
          <tbody>
            {linkHistory.map((link, index) => (
              ...
              >
                ...
                <td className="p-4 border-b border-blue-gray-50">
                  <Link
                    to={`https://scissor-mbob.onrender.com/${link.shortened_url}`}
                    target="_blank"
                  >
                    <Typography
                      variant="small"
                      className="font-normal text-blu hover:text-toma"
                    >
                      {link.shortened_url}
                    </Typography>
                  </Link>
